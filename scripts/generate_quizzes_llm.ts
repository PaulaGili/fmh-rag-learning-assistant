/**
 * Generate exam-style MCQ questions from FMH knowledge chunks using LLM.
 *
 * Processes each category separately, merges with existing quizzes,
 * and writes to data/quizzes.json.
 *
 * Usage: npx tsx scripts/generate_quizzes_llm.ts
...
function buildPrompt(catDisplay: string, chunks: FmhEntry[], count: number): string {
  ...
  return `You are a medical exam question writer for the Swiss FMH gynecology specialist exam.

Given the following study material for the category "${catDisplay}", generate exactly ${count} high-quality multiple-choice questions.
...
- Write questions in English (matching the source material language)
